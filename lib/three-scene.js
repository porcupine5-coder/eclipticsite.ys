...
class HeroScene {
  constructor() {
    ...
    this.isDark = this.themeRoot.getAttribute('data-theme') === 'dark';
  ...
  createFluidPlane() {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        if (uIsDark > 0.5) {
          // Dark mode - electric blue + deep violet on black base
          vec3 color1 = vec3(0.02, 0.02, 0.06); // Base black (lifted slightly)
          vec3 color2 = vec3(0.05, 0.82, 1.00); // Electric blue
          vec3 color3 = vec3(0.26, 0.06, 0.50); // Deep violet
          vec3 color4 = vec3(0.00, 0.52, 0.96); // Saturated blue accent
          
          color = mix(color1, color2, gradient1 * 0.55);
          color = mix(color, color3, gradient2 * 0.5);
          color = mix(color, color4, gradient3 * 0.35);
          
          // Cool electric mouse glow
          color += vec3(0.18, 0.62, 1.00) * mouseEffect * 0.45;
        } else {
          // Light mode - Liquid fluid palette (violet/orange with stronger presence)
          vec3 color1 = vec3(0.95, 0.96, 0.99); // Soft white
          vec3 color2 = vec3(0.60, 0.45, 0.98); // Violet
          vec3 color3 = vec3(0.98, 0.55, 0.25); // Orange
          vec3 color4 = vec3(0.88, 0.92, 1.0);  // Light blue
          
          color = mix(color1, color2, gradient1 * 0.5);
          color = mix(color, color3, gradient2 * 0.45);
          color = mix(color, color4, gradient3 * 0.5);
          
          // Stronger noise-based variation for liquid feel
          color += vec3(noise1, noise2, (noise1 + noise2) * 0.5) * 0.06;
          
          // Add mouse glow
          color += vec3(0.55, 0.45, 0.95) * mouseEffect * 0.35;
        }
  ...
  addEventListeners() {
    ...
    // MutationObserver to watch for theme changes on the theme root (html)
    ...
    this.observer.observe(this.themeRoot, {
      attributes: true,
